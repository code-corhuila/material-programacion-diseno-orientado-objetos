/* =========================================================
   OVA App · Programación Móvil · CORHUILA
   =========================================================
...
(function () {
  ...
  const TAB_STORAGE_KEY = "ova_programacion_movil_tab_v2";
  /* ---------------------------------------------------------
  ...
  const UNITS = [
  { id: "u1", title: "Unidad 1", meta: "Semanas 1–5 · (Incluye Primer Parcial)", from: 1, to: 5 },
  { id: "u2", title: "Unidad 2", meta: "Semanas 6–10 · (Incluye Segundo Parcial)", from: 6, to: 10 },
  { id: "u3", title: "Unidad 3", meta: "Semanas 11–16 · (Cierre y evaluación final)", from: 11, to: 16 }
];

function unitForWeek(week) {
  return UNITS.find(u => week >= u.from && week <= u.to) || null;
}

function renderUnitHeader(unit) {
  return `
    <div class="unit-hd" role="heading" aria-level="3"
         data-from="${unit.from}" data-to="${unit.to}" data-unit="${unit.id}">
      <div class="unit-hd__left">
        <span class="unit-hd__pill">${escapeHtml(unit.title)}</span>
        <div class="unit-hd__meta">${escapeHtml(unit.meta)}</div>
      </div>
    </div>
  `;
}

/* Oculta/mostrar headers según lo que quede visible tras filtrar */
function updateUnitHeadersVisibility() {
  qsa(".unit-hd").forEach((hd) => {
    const from = parseInt(hd.dataset.from, 10);
    const to = parseInt(hd.dataset.to, 10);

    const itemsInRange = qsa(".item").filter((it) => {
      const w = parseInt(it.dataset.week, 10);
      return w >= from && w <= to;
    });

    const anyVisible = itemsInRange.some((it) => it.style.display !== "none");
    hd.style.display = anyVisible ? "" : "none";
  });
}
  ...
  // Storage keys
  const STORAGE_KEYS = {
    progress: "ova_programacion_movil_progress_v2",
    quiz: "ova_programacion_movil_quiz_v2"
  };
  ...
  function inferActivityTag(text) {
    ...
    if(lowered.includes("actividad")) return "actividad";
    if (lowered.includes("proyecto")) return "proyecto";
    return "otro";
  }

  function stripLeadingActivityPrefix(text) {
  return (text || "").replace(/^\s*(foro|quiz|taller|parcial|proyecto|actividad)\s*:\s*/i, "");
}
...
let _confirmResetResolver = null;
let _confirmResetLastFocus = null;

function confirmResetProgress() {
  const modal = qs("#confirmReset");
  const okBtn = qs("#confirmResetOk");
  if (!modal || !okBtn) return Promise.resolve(true); // fallback por si falta el modal

  _confirmResetLastFocus = document.activeElement;

  modal.hidden = false;
  document.body.classList.add("modal-open");

  // Llevar foco al botón principal
  setTimeout(() => okBtn.focus(), 0);

  return new Promise((resolve) => {
    _confirmResetResolver = resolve;
  });
}

function closeConfirmReset(result) {
  const modal = qs("#confirmReset");
  if (!modal) return;
  ...
  if (typeof _confirmResetResolver === "function") {
    _confirmResetResolver(!!result);
  }
  _confirmResetResolver = null;

  if (_confirmResetLastFocus && typeof _confirmResetLastFocus.focus === "function") {
    _confirmResetLastFocus.focus();
  }
  _confirmResetLastFocus = null;
}
  ...
  const ok = await confirmResetProgress();
  if (!ok) return;
...
function renderSessions() {
  ...
  const sorted = sessions.slice().sort((a, b) => (a.semana || 0) - (b.semana || 0));
  ...
  const html = sorted.map((session) => {
    ...
    const u = unitForWeek(session.semana);
    let unitHeaderHtml = "";
    if (u && u.id !== lastUnitId) {
      unitHeaderHtml = renderUnitHeader(u);
      lastUnitId = u.id;
    }
        ...
        <div class="item__panel" id="${panelId}" hidden>
          <div class="item__grid">
            ...
            <div class="kv">
              ...
              <div class="kv__v">
              <em>${getTagLabel(tag)}:</em> ${escapeHtml(stripLeadingActivityPrefix(session.autonoma))}
    ...
    return unitHeaderHtml + weekHtml;
  }).join("");
  ...
  // Asegura headers correctos al inicio
  updateUnitHeadersVisibility();
}
...
function initMobileMenu() {
  ...
  // Close menu when clicking a link + cambiar tab + scroll
  qsa("a", mobileMenu).forEach((link) => {
    on(link, "click", (e) => {
      const tab = link.dataset.tab;           // ini, just, obj, ruta, eval, rec...
      const href = link.getAttribute("href"); // #ruta, #objetivos, etc.

      // Si tiene tab definido, primero mostramos el panel correcto
      if (tab) {
        e.preventDefault();
        showTab(tab, false);
        ...
        // hacer scroll cuando ya se mostró el tab
        setTimeout(() => {
      ...
      // si no tiene data-tab, solo cerrar el menú normal
      menuButton.setAttribute("aria-expanded", "false");
      mobileMenu.hidden = true;
    });
  });
...
const modal = qs("#confirmReset");
const okBtn = qs("#confirmResetOk");
const cancelBtn = qs("#confirmResetCancel");
const backdrop = qs("#confirmResetBackdrop");

if (modal && okBtn && cancelBtn && backdrop) {
  on(okBtn, "click", () => closeConfirmReset(true));
  on(cancelBtn, "click", () => closeConfirmReset(false));
  on(backdrop, "click", () => closeConfirmReset(false));
  ...
  on(document, "keydown", (e) => {
    ...
    // Botón "Iniciar ruta" -> cambia a pestaña Ruta y baja al bloque
const btnStartRuta = qs("#btnStartRuta");
if (btnStartRuta) {
  on(btnStartRuta, "click", (e) => {
    e.preventDefault();

    // 1) Mostrar pestaña Ruta
    showTab("ruta", false);

    // 2) Scroll suave al bloque ruta (ya visible)
    setTimeout(() => {
      const el = qs("#ruta");
